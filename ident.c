...
static unsigned short int CalculateCRCOfROM(void *buffer1, void *buffer2, void *start, unsigned int length)
{
	...
	for(i = 0,prevSize = size,crc = CRC16_INITIAL_CHECKSUM,pDestBuffer = buffer1,pSrcBuffer = start; i < length; i+=size, pSrcBuffer+=size)
	{
		...
		SysmanSync(0);
		while(SysmanReadMemory(pSrcBuffer, pDestBuffer, size, 1) != 0) nopdelay();
...
int CheckROM(const struct PS2IDBMainboardEntry *entry)
{
	...
	{
		if((entry->BOOT_ROM.IsExists && (other->BOOT_ROM.crc16 != entry->BOOT_ROM.crc16))
		   || (other->DVD_ROM.IsExists && (other->DVD_ROM.crc16 != entry->DVD_ROM.crc16)))
		{
...
int DumpRom(const char *filename, const struct SystemInformation *SystemInformation, struct DumpingStatus *DumpingStatus, unsigned int DumpingRegion){
	...
	if((file=fopen(filename, "wb"))!=NULL){
		for(pBuffer = buffer1,prevSize = BytesRemaining; BytesRemaining>0; MemDumpStart+=BytesToRead,BytesRemaining-=BytesToRead){
			...
			SysmanSync(0);
			while(SysmanReadMemory(MemDumpStart, pBuffer, BytesToRead, 1) != 0) nopdelay();
...
const char *GetSPEEDCapsDesc(unsigned short int caps){
	...
	if(caps!=0){
		...
		for(i=0,NumCapabilities=0; i<8; i++){
			if(caps>>i&1){
				if(NumCapabilities>0) strcat(capsbuffer, ", ");
...
int WriteSystemInformation(FILE *stream, const struct SystemInformation *SystemInformation){
	...
	fprintf(stream, "Log file generated by Playstation 2 Ident v"PS2IDENT_VERSION", built on "__DATE__" "__TIME__"\r\n\r\n");
	fprintf(stream, "ROMVER:\t%s\r\n", SystemInformation->mainboard.romver);
	fprintf(stream, "ROM region sizes:\r\n");
	for(i=0; i<=2; i++){
		fprintf(stream, "\tROM%u:\t", i);
		if(SystemInformation->ROMs[i].IsExists) fprintf(stream, "%p (%u bytes)\r\n", SystemInformation->ROMs[i].StartAddress, SystemInformation->ROMs[i].size);
		else fprintf(stream, "<Not detected>\r\n");
	}
	fprintf(stream, "\tEROM:\t");
	if(SystemInformation->erom.IsExists) fprintf(stream, "%p (%u bytes)\r\n", SystemInformation->erom.StartAddress, SystemInformation->erom.size);
	else fprintf(stream, "<Not detected>\r\n");
	...
	if(SystemInformation->mainboard.ssbus.status & PS2DB_SSBUS_HAS_AIF)
		fprintf(stream, "%u", SystemInformation->mainboard.ssbus.AIFRevision);
	else
	...
	fprintf(stream, "Mainboard:\r\n"
		...
		"\tMECHACON revision:\t%u.%u (%s)\r\n"
		"\tMagicGate region:\t0x%02x (%s)\r\n"
		"\tSystem type:\t\t0x%02x (%s)\r\n"
		"\tADD0x010:\t\t0x%04x (%s)\r\n"
		"\tM Renewal Date:\t\t",	SystemInformation->mainboard.ModelName, SystemInformation->mainboard.MainboardName, SystemInformation->chassis,
						SystemInformation->mainboard.ROMGEN_MonthDate, SystemInformation->mainboard.ROMGEN_Year, SystemInformation->mainboard.MachineType, SystemInformation->mainboard.BoardInf, GetMRPDesc(SystemInformation->mainboard.BoardInf), SystemInformation->mainboard.MPUBoardID, SystemInformation->mainboard.spu2.revision, GetSPU2ChipDesc(SystemInformation->mainboard.spu2.revision),
	...
	if(SystemInformation->mainboard.MECHACONVersion[1]<5 || (SystemInformation->mainboard.status&PS2IDB_STAT_ERR_MRENEWDATE))
		...
		fprintf(stream, "20%02x/%02x/%02x %02x:%02x", SystemInformation->mainboard.MRenewalDate[0], SystemInformation->mainboard.MRenewalDate[1], SystemInformation->mainboard.MRenewalDate[2], SystemInformation->mainboard.MRenewalDate[3], SystemInformation->mainboard.MRenewalDate[4]);
	fprintf(stream, "\r\n");
	...
	fprintf(stream,
		...
		"\tGS ID:\t\t\t0x%02x\r\n",
						...
						conModelID, SystemInformation->mainboard.EMCSID,
