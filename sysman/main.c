...
int SysmanGetHardwareInfo(t_SysmanHardwareInfo *hwinfo){
	...
	hwinfo->EE_F520=*(volatile unsigned int *)0xB000F520;
	hwinfo->EE_F540=*(volatile unsigned int *)0xB000F540;
	hwinfo->EE_F550=*(volatile unsigned int *)0xB000F550;
	...
	switch(fno){
		case SYSMAN_ReadMemory:
			...
			while(BytesRemaining>0){
				...
				if((*(int *)ResultBuffer=SysmanReadMemory(source, IOBuffer[BufferID], BytesToRead))==0){
					...
					if(old_dmat_id>=0) while(sceSifDmaStat(old_dmat_id)>=0){};	//Do not overwrite data that has not been transferred over to the EE.
				}
				else break;
		...
		case SYSMAN_WriteMemory:
			...
			while(BytesRemaining>0){
				...
				if((*(int *)ResultBuffer=SysmanWriteMemory(destination, IOBuffer, BytesToRead))!=0) break;
