...
int LoadROMImg(ROMIMG *ROMImg, const char *path)
{
    FILE *InputFile;
    int result;
    unsigned int i, offset, ExtInfoOffset, DataStartOffset, ScanLimit;
    // unsigned int CommentLength;
    ...
    if ((InputFile = fopen(path, "rb")) != NULL)
    {
        ...
        if ((ImageStat.image = malloc(ImageStat.size)) != NULL)
        {
            if (fread(ImageStat.image, 1, ImageStat.size, InputFile) == ImageStat.size)
            {
                ...
                /* Scan for the start of the image, which is after the end of the bootstrap program.
                   All regular IOPRP images don't have one (The image begins immediately at the start of the file). */
                ...
                if ((ROMImg->files = (ROMImg->files == NULL) ? malloc(sizeof(struct FileEntry)) : realloc(ROMImg->files, ROMImg->NumFiles * sizeof(struct FileEntry))) != NULL)
                {
                    ...
                    if (IsSonyRXModule(path))
                    {
                        if ((result = GetSonyRXModInfo(path, ModuleDescription, sizeof(ModuleDescription), &FileVersion)) == 0)
                        {
                            ...
                            AddExtInfoStat(&ROMImg->files[ROMImg->NumFiles - 1], EXTINFO_FIELD_TYPE_COMMENT, ModuleDescription, (unsigned char)strlen(ModuleDescription) + 1);
                        }
                    }
                    else
                        result = 0;
