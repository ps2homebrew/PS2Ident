...
static int PS2IDBMS_LoadMoboDatabase(FILE *DatabaseFile, unsigned short int NumEntries)
{
    ...
    if ((MoboDatabase = malloc(NumEntries * sizeof(struct PS2IDBMainboardEntry))) != NULL)
    {
        if (fread(MoboDatabase, sizeof(struct PS2IDBMainboardEntry), NumEntries, DatabaseFile) == NumEntries)
        {
            ...
            { //Clear the KSEG1 bits from the physical addresses.
                MoboDatabase[i].BOOT_ROM.StartAddress &= ~0xA0000000;
                MoboDatabase[i].DVD_ROM.StartAddress &= ~0xA0000000;
            }
...
{
    int result, i;
    struct PS2IDBMainboardEntry112 *MoboDatabase;
    struct PS2IDBMainboardEntry *NewMoboDatabase;
    ...
    if (MoboDatabase != NULL && NewMoboDatabase != NULL)
    {
        if (fread(MoboDatabase, sizeof(struct PS2IDBMainboardEntry112), NumEntries, DatabaseFile) == NumEntries)
        {
            ...
            for (i = 0; i < NumEntries; i++)
            {
                memset(&NewMoboDatabase[i], 0, sizeof(struct PS2IDBMainboardEntry));
                memcpy(&NewMoboDatabase[i], &MoboDatabase[i], sizeof(struct PS2IDBMainboardEntry112));

                printf("\t%d.\t%s (%s) - Contributor: ", i + 1, NewMoboDatabase[i].ModelName, NewMoboDatabase[i].MainboardName);
                do
                {
                    GetString(ContributorName, sizeof(ContributorName));
                } while (strlen(ContributorName) < 1);

                strncpy(NewMoboDatabase[i].ContributorName, ContributorName, sizeof(NewMoboDatabase[i].ContributorName) - 1);
                NewMoboDatabase[i].ContributorName[sizeof(NewMoboDatabase[i].ContributorName) - 1] = '\0';
            }

            peripherals[PS2IDB_COMPONENT_MAINBOARD].models    = NewMoboDatabase;
            peripherals[PS2IDB_COMPONENT_MAINBOARD].NumModels = NumEntries;
            result                                            = 0;
        }
        else
...
static int PS2IDBMS_LoadDatabase0112(struct PS2IDBHeader *header, FILE *DatabaseFile)
{
    ...
    printf("Converting old database...");
                    ...
                    if (component.id < PS2IDB_COMPONENT_COUNT && component.entries > 0)
                    {
                        switch (component.id)
                        {
                            case PS2IDB_COMPONENT_MAINBOARD:
                                result = PS2IDBMS_LoadMoboDatabase0112(DatabaseFile, component.entries);
                                break;
                            default:
...
int PS2IDBMS_LoadDatabase(const char *path)
{
    ...
    if ((DatabaseFile = fopen(path, "rb")) != NULL)
    {
        if (fread(&header, sizeof(header), 1, DatabaseFile) == 1 && (strncmp(header.magic, "P2DB", 4) == 0))
        {
            ...
            {
                case PS2IDB_FORMAT_VERSION_0112:
                    result = PS2IDBMS_LoadDatabase0112(&header, DatabaseFile);
                    break;
...
int PS2IDBMS_AddMainboardModel(const struct PS2IDBMainboardEntry *entry)
{
    ...
    if (result == 0)
    {
        if ((database = realloc(database, (NumModels + 1) * sizeof(struct PS2IDBMainboardEntry))) != NULL)
        {
            ...
            //Clear the KSEG1 bits from the physical addresses.
            database[NumModels].BOOT_ROM.StartAddress &= ~0xA0000000;
            database[NumModels].DVD_ROM.StartAddress &= ~0xA0000000;
